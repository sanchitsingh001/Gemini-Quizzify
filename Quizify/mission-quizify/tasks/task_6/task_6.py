...
if __name__ == "__main__":
    ...
    with screen.container():
        ...
        docProcessor.ingest_documents()
        # 2) Initalize the EmbeddingClient from Task 4 with embed config
        embedClient = EmbeddingClient(embed_config["model_name"], embed_config["project"],embed_config["location"])
        # 3) Initialize the ChromaCollectionCreator from Task 5
        chromo = ChromaCollectionCreator(processor=docProcessor, embed_model=embedClient)
        ####### YOUR CODE HERE #######
        ...
        with st.form("Load Data to Chroma"):
            ...
            if submitted:
                ...
                ####### YOUR CODE HERE #######
                chroma_collection = chromo.create_chroma_collection()

                # Uncomment the following lines to test the query_chroma_collection() method
                document = chromo.query_chroma_collection(topic_input) 
