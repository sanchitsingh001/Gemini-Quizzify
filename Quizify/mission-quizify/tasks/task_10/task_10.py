...
if __name__ == "__main__":
    ...
    embed_config = {
        ...
        "project": "mission-quizify-424218",
    ...
    if 'question_bank' not in st.session_state or len(st.session_state['question_bank']) == 0:
        ...
        with screen.container():
            ...
            with st.form("Load Data to Chroma"):
                ...
                questions = st.number_input("Number of questions:", min_value=1, max_value=50, value=3, step=1)
                ##### YOUR CODE HERE #####
                ...
                if submitted:
                    ...
                    question_bank = generator.generate_quiz()# Step 3: Initialize a QuizGenerator class using the topic, number of questrions, and the chroma collection
        ...
        with st.container():
            ...
            def next_question_callback():
                    if st.session_state['question_index'] + 1 < len(st.session_state['question_bank']):
                        st.session_state['question_index'] += 1

            def prev_question_callback():
                if st.session_state['question_index'] - 1 >= 0:
                    st.session_state['question_index'] -= 1

            next_question = st.button("Next Question", on_click= next_question_callback)
            previous_question = st.button("Previous Question", on_click=prev_question_callback)
